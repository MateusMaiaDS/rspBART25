...
nodeLogLike <- function(curr_part_res,
  ...
  for(jj in 1:length(j_)){
      ...
      if(j_[jj] <= length(data$dummy_x$continuousVars)){
        cov_aux <- cov_aux + (data$tau_beta[j_[jj]]^(-1))*node_basis_list[[jj]][,,drop = FALSE]%*%tcrossprod(data$P_inv,(node_basis_list[[jj]][,,drop = FALSE]))
      } else {
        cov_aux <- cov_aux + (data$tau_beta[j_[jj]]^(-1))*node_basis_list[[jj]][,,drop = FALSE]%*%tcrossprod(data$P_inv_interaction,(node_basis_list[[jj]][,,drop = FALSE]))
      }
  }

  # Adding the main diagonal
  cov_aux <- (diag(nrow = n_leaf) + cov_aux)/data$tau + 1/data$tau_gamma
